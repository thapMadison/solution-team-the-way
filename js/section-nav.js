 ...
 *
 * Each item's data-section must match an element id in the document.
 *
...
(function () {
  ...
  function initKeyboardJump(sectionsByRail) {
    // Use the first rail's sections for keyboard shortcuts.
    const sections = sectionsByRail[0];
    if (!sections) return;
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (!armed && (e.key === 'g' || e.key === 'G')) {
        ...
        armTimer = setTimeout(disarm, 1200);
        return;
      }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    initKeyboardJump(sectionsByRail);
  });
})();
